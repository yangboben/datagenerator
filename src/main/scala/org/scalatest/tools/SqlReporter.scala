...
/**
 * Created by tianming on 15-11-18.
 */
...
import io.transwarp.streaming.tester.StreamSQLTester
 ...
 */
class SqlReporter() extends Reporter {
  val directory = StreamSQLTester.loadProps().getProperty(StreamSQLTester.REPORT)
...
//      }
  ...
  def getInfos(recordedEvents: collection.immutable.IndexedSeq[RecordableEvent]): Array[String] = {
    var infos: Array[String] = Array()
    recordedEvents.foreach( e => {
      e match {
        case e: InfoProvided => infos :+= e.message
        case _ => {}
      }
    })
    infos
    ...
    while ((idx < orderedEvents.size) && (endIndex == 0)) {
      ...
      event match {
        case e: TestSucceeded =>
          ...
          testcase.infos = getInfos(e.recordedEvents)
        ...
        case e: ScopePending   => idx += 1
        case e: InfoProvided   =>
          testcase.infos :+= e.toString
          idx += 1
        case e: MarkupProvided => idx += 1
        case e: AlertProvided  =>
          testcase.alerts :+= e.toString
          idx += 1
        case e: NoteProvided   => idx += 1
        ...
        val (throwableType, throwableText) =
          failure.throwable match {
            ...
            case Some(throwable) =>
              ...
              val throwableText = throwable.getMessage
